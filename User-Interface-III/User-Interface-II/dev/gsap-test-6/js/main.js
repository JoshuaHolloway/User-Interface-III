...
const do_animation = (e) => {
  if (is_even(count)) {
    ...
    .to('.line-3', {
      ...
      transformOrigin: "50% 50%", 
      rotation: 0}, 2*dur1)
    .to('.box',        {duration: dur1, y: 0, stagger: {each: 0.3}}, 3*dur1)
    .to('.line-2',     {duration: dur1, opacity: 1},  4*dur1)
    .to('.line-1',     {duration: dur1, y: 0},        4*dur1)
    .to('.line-3',     {duration: dur1, y: 0},        4*dur1)
    .to('.nav-drawer', {duration: dur1, xPercent: 0}, 4*dur1);
  }
  count++;
};
